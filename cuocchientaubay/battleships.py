...
BACKGROUND = pygame.transform.scale(pygame.image.load(os.path.join('Assets', 'SPACE_BG.gif')), (WIDTH, HEIGHT ))

FIRE = pygame.mixer.Sound(os.path.join('Assets', "Gun+Silencer.mp3"))
CRASH = pygame.mixer.Sound(os.path.join('Assets', "Grenade+1.mp3"))
BACROUNDMS = pygame.mixer.Sound(os.path.join('Assets', "BRMS.mp3"))
...
def draw_winner(text,win):
    if win == 1 : draw_text = WINNER_FONT.render(text, 1, YELLOW)
    else : draw_text = WINNER_FONT.render(text, 1, RED)
...
def main():
    ...
    while run: #main loop
        ...
        if red_health <= 0:
            ...
            draw_winner(winner_txt,1)
            BACROUNDMS.stop()
            break
            
        if yellow_health <= 0:
            winner_txt = "RED  WIN  !"
            draw_winner(winner_txt,2)
            BACROUNDMS.stop()
            break
        """    
        if winner_txt != "":
